...
begin
  ...
  [
    ...
    TAG_(MUIA_Application_Base)         , TAG_(PChar('EXAMPLE7A')),
    TAG_(MUIA_Application_Copyright)    , TAG_(PChar('© 1999 by BlaBla Corp.')),
    TAG_(MUIA_Application_Description)  , TAG_(PChar('Example 7a to the MUI tutorial')),
    TAG_(MUIA_Application_Title)        , TAG_(PChar('Example7a')),
    TAG_(MUIA_Application_Version)      , TAG_(PChar('$VER: example7a 1.0 (2.9.1999) BLABLA PRODUCT')),
    TAG_(MUIA_Application_Window)       , TAG_(SetAndGet(Win, MUI_NewObject (MUIC_Window,
    [
      TAG_(MUIA_Window_Title)           , TAG_(PChar('Example 7a')),
      TAG_(MUIA_Window_ID)              , $50525A4B,
      TAG_(MUIA_UserData)               , TAG_(OBJ_WINDOW),
      TAG_(MUIA_Window_RootObject)      , TAG_(MUI_NewObject (MUIC_Group,
      [
        TAG_(MUIA_Group_Child)          , TAG_(MUI_NewObject (MUIC_Group,       //* the first group */
        [                                                                       //* without MUIA_Text_SetMax   */
          TAG_(MUIA_Group_Horiz)        , TAG_(TRUE),                           
          TAG_(MUIA_Group_Child)        , TAG_(MUI_NewObject (MUIC_Text,
          [
            TAG_(MUIA_Text_Contents)    , TAG_(PChar(Esc_R + 'without MUIA_Text_SetMax')),  //* aligned to the right */
            TAG_END
          ])),
        ...
        TAG_(MUIA_Group_Child)          , TAG_(MUI_NewObject (MUIC_Group,       //* the second group */
        [                                                                       //* with MUIA_Text_SetMax   */
          TAG_(MUIA_Group_Horiz)        , TAG_(TRUE),
          TAG_(MUIA_Group_Child)        , TAG_(MUI_NewObject (MUIC_Text,
          [
            TAG_(MUIA_Text_Contents)    , TAG_(PChar(Esc_R + 'with MUIA_Text_SetMax')),    //* aligned to the right */
