...

{
  ===========================================================================
  Project : example15b
  Topic   : Prevent error on closing by using the button instead
...
begin
  ...
  [
    ...
    [
      TAG_(MUIA_Window_Title)       , TAG_(PChar('Example 15b')),
  ...
  begin
    ...
    [
      TAG_(MUIA_Selected), TAG_(TRUE), TAG_(_obj), 
      1, TAG_(APPM_AddSubWindow)
    ]);

    DoMethod (_button, MUIM_Notify, 
    [
      TAG_(MUIA_Selected), TAG_(FALSE), TAG_(_obj), 
...
begin
  ...
  MUI_DisposeObject (data^.subwindow);
  data^.subwindow := nil;
  result := 0;
...
begin
  ...
  [
    ...
    TAG_(MUIA_Application_Base)         , TAG_(PChar('EXAMPLE15B')),
    TAG_(MUIA_Application_Copyright)    , TAG_(PChar('© 2000 by Grzegorz Kraszewski')),
    TAG_(MUIA_Application_Description)  , TAG_(PChar('Example 15b to the MUI tutorial')),
    TAG_(MUIA_Application_Title)        , TAG_(PChar('Example15b')),
    TAG_(MUIA_Application_Version)      , TAG_(PChar('$VER: example15b 1.0 (25.11.2000)')),
    TAG_END
  ]);
