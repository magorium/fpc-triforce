program notification_with_hooks;
...
{
  ===========================================================================
  Project : notification with hooks
  Topic   : MUI notifications using hooks
...
Function TestThirteen(hook: PHook; obj: pObject_; data: pLong): LONG;
begin
  if (Data^ = 13) 
  then SetAttrs (TxtTxt, [TAG_(MUIA_Text_Contents), TAG_(PChar('Unlucky number!')), TAG_END])
  else SetAttrs (TxtTxt, [TAG_(MUIA_Text_Contents), TAG_(PChar('')), TAG_END]);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        InitHook(h_TestThirteen, THookFunction(@TestThirteen), nil);

        DoMethod (Slider, MUIM_Notify, 
        [
          TAG_(MUIA_Slider_Level), TAG_(MUIV_EveryTime), TAG_(TxtTxt), 
          3, TAG_(MUIM_CallHook), TAG_(@h_TestThirteen), TAG_(MUIV_TriggerValue)
        ]);
