...
function  MerchandiseDisplayer(hook: pHook; tekst: ppChar; merchandise: PMerchandise): ULONG;
Const
  textl    : ShortString = '';
begin
  if (merchandise^.items_in_stock > 0) 
  then System.WriteStr(textl,        merchandise^.name, ' ', merchandise^.items_in_stock       , #0)
  else System.WriteStr(textl, Esc_B, merchandise^.name, ' ', merchandise^.items_in_stock, Esc_N, #0);

  tekst[0] := @textl[1];    { FPC Note: Return Pascal string as PChar  }
  result := 0;
end;
...
begin
  ...
  [
    ...
    TAG_(MUIA_Application_Base)             , TAG_(PChar('EXAMPLE10A')),
    TAG_(MUIA_Application_Copyright)        , TAG_(PChar('© 1999 by BlaBla Corp.')),
    TAG_(MUIA_Application_Description)      , TAG_(PChar('Example 10a to the MUI tutorial')),
    TAG_(MUIA_Application_Title)            , TAG_(PChar('Example10a')),
    TAG_(MUIA_Application_Version)          , TAG_(PChar('$VER: example10a 1.0 (10.2.2000) BLABLA PRODUCT')),
    TAG_(MUIA_Application_Window)           , TAG_(SetAndGet(Win, MUI_NewObject (MUIC_Window,
    [
      TAG_(MUIA_Window_Title)               , TAG_(PChar('Example 10a')),
      ...
      [
        ...
        [
          ...
          [
            ...
            TAG_(MUIA_List_Title)           , TAG_(PChar(#27'1' + Esc_B + 'Default table')), //* Title text */
...
begin  
  ...
  [ 
    TAG_(@amigas), TAG_(MUIV_List_Insert_Top)
  ]);
